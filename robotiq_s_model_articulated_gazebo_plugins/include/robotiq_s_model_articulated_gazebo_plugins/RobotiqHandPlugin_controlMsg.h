...
// We use this plugin instead of `RobotiqHandPlugin.h` so that we will publish msg type = `robotiq_s_model_control/SModel_robot_input` and `robotiq_s_model_control/SModel_robot_output` to topics `/SModelRobotInput` and `/SModelRobotOutput` to control BOTH the real and simulated gripper. 
// If we use `RobotiqHandPlugin.h`, then we are publishing the msg type = `robotiq_s_model_articulated_msgs/SModelRobotInput` and `robotiq_s_model_articulated_msgs/SModelRobotOutput`. This is fine with simulation, but not real run. Not sure though.


#ifndef GAZEBO_ROBOTIQ_HAND_PLUGIN_CONTROL_MSG_HH
#define GAZEBO_ROBOTIQ_HAND_PLUGIN_CONTROL_MSG_HH
...
///                   This parameter is optional.
class RobotiqHandPluginControlMsg : public gazebo::ModelPlugin
{
  ...
  /// \brief Constructor.
  public: RobotiqHandPluginControlMsg();

  /// \brief Destructor.
  public: virtual ~RobotiqHandPluginControlMsg();
  ...
  private: static const std::string left_side;
  private: static const std::string right_side;
  private: static const std::string left_prefix;
  private: static const std::string right_prefix;

  private: static const std::string DefaultLeftTopicJointStates;
  private: static const std::string DefaultRightTopicJointStates;
